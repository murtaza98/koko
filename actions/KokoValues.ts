...
import { IValuesPointStorage, IValueAnswerStorage, IValueAnswer, IMonthlyValuesStorage } from '../storage/IValuesStorage';
...
export class KokoValues {
    ...
    private async sendValuesAnswer(read: IRead, modify: IModify, persistence: IPersistence, sender: IUser, http: IHttp, dots: Array<string>, text: string, usernames?: Array<string>) {
        ...
        let monthlyValues = monhtlyValuesData && monhtlyValuesData.length > 0 && monhtlyValuesData[0] as IMonthlyValuesStorage;
        if (!monthlyValues) {
            monthlyValues = {};
        }
